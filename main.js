...
const vatRates   = require('./vat-rates').rates;
...
function vatman() {
  return through.obj(function(chunk, enc, cb) {
    ...
    const records = chunk.records.map(function(record, i) {
      ...
      const vatRate     = new BigNumber(vatRates[countryCode].standard_rate).div(100);
    ...
    this.push(`${records.length} PayPal-Bestellungen im ${chunk.quarter}. Quartal ${chunk.year}\n\n`);

    spaceAlign([dataHeadings].concat(records), { spacing: 3 })
      .forEach(function(row) { this.push(row + '\n'); }.bind(this));
